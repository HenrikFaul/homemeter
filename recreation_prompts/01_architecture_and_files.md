# Architektúra és teljes fájlterv

Dependency-sorrendben (config/db/típusok előbb, belépési pont és tesztek a végén):
...
- `src/routes/api/workOrdersRoute.ts` — Express route handlers mounted at /api/work_orders endpoint for creating, updating, and listing maintenance tasks with filters like severity or status. — KÖTELEZŐ exportok (más fájlok PONTOSAN ezeket importálják, ne nevezd át őket): workOrdersRoutes
...
Build: `npm install --no-audit --no-fund` → `npx --yes tsc -p tsconfig.json --noEmit` → `npx --yes tsc -p tsconfig.json` → start: `node dist/server.js`